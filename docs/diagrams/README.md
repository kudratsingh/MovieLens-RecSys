...
| [`tenancy-and-auth`](tenancy-and-auth.svg) | The isolation mechanism: issuer → tenant, the impersonation gate, the four Postgres identities, pgBouncer's transaction pool, the eight forced-RLS tables against the deliberately shared ones, and the two cross-tenant canaries. |
