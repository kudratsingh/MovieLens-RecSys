...
| 3 — Target attention ranker | approved; **increment 1 measured and refused, increment 2 not built** | 2026-09-05 | 0018 | Approved by the owner 2026-09-05. **Increment 1** gave the learned-route booster the frozen encoder's score against each candidate as two point-in-time columns (`eee531bb16d943f5a2213dd9b7a8dc1a`, ten-column booster `7eba8851926bc88d…`), judged against PR #151's per-route bundle (`566f5309767a4076a4f5e8151be16645`) on identical positives, identical groups and a bit-identical cold slice. Warm NDCG@10 **+0.94%** (0.091688 -> 0.092550), cold **0.00%**, overall +0.29%, warm recall@10 **−1.50%**. Both gate readings refuse and agree, so O-1 does not decide it. **Stop rule 1 fired** at under +3% warm, and the approval amendment's first precondition therefore failed, so **increment 2 (DIN) is not built**. The diagnosis is ADR 0018's own Risk 1, predicted before the run: the two features took **77.5% of the model's total gain** while every aggregate fell, which is what "the dot product re-encodes the retriever's rank" looks like — the candidates *are* SASRec's top-500, so the score is a monotone function of the rank within a group. What stands is narrower than the rung's premise: the encoder's *scalar verdict* adds nothing to a ranker scoring the encoder's *own* candidates. Both arms were measured under O-9 (a left-padded sequence encodes to NaN, so 34,190 of 153,947 learned-route positives retrieve nothing); the comparison is internally valid, the re-measurement is work item W17 |
