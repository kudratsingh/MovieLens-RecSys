...
| 3 — Target attention ranker | **approved** | 2026-09-05 | 0018 | Two increments. **1:** the frozen SASRec encoder's user embedding and its score against each candidate as point-in-time LightGBM features, judged against PR #151's per-route bundle (`566f5309767a4076a4f5e8151be16645`; warm 0.091688 / cold 0.549002 / overall 0.214631) — cold is bit-identical by construction, so the claim is warm. **2:** DIN target attention in the sidecar, under an isolated p99 <10 ms budget at 500 candidates, and not built unless increment 1 gains at least **+3% warm NDCG@10** over the per-route bundle *and* the owner names a warm NDCG@10 target once increment 1's number exists. Approved by the owner 2026-09-05 |
