...
| 2 — SASRec | done — measured, not promoted | 2026-09-05 | 0016 | Retrieval passed (+16.57% warm recall; encoder p99 0.285 ms), but paired LightGBM improved overall NDCG@10 only **+0.43%** against the required +3% (+1.67% warm, 0.00% cold). Exact encoder/ranker artifacts, evidence, and logs retained; item-item remains the two-stage retrieval source |
