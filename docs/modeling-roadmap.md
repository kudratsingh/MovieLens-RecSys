...
| 3 — Target attention ranker | **proposed** | 2026-09-05 | 0018 | Two increments. **1:** the frozen SASRec encoder's user embedding and its score against each candidate as point-in-time LightGBM features, judged against PR #151's per-route bundle (warm 0.091688 / cold 0.549002 / overall 0.214631) — cold is bit-identical by construction, so the claim is warm. Stop at <+3% warm NDCG@10. **2:** DIN target attention in the sidecar, started only on a gap increment 1 names, under an isolated p99 <10 ms budget at 500 candidates. Awaiting owner approval; no training code until this row says approved |
