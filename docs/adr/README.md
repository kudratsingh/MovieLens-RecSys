...
| 0017 | [Content-Based Cold-Item Retrieval](0017-content-based-cold-item-retrieval.md) | A second candidate source representing items by attributes rather than by who consumed them, because 3,376 catalog movies have zero ratings and no interaction-derived index can reach them. Genres and release year first; the tag genome covers 0% of cold items and is explicitly not the answer. | Accepted | Modeling ladder |
| 0018 | [A Ranker That Reads the Sequence](0018-sequence-aware-ranking.md) | Roadmap Rung 3, approved 2026-09-05 in two increments: expose the frozen SASRec encoder's user embedding and its score against each candidate as point-in-time LightGBM features, and only on a named gap add DIN-style target attention in the sidecar. Judged against PR #151's per-route SASRec bundle, not against item-item plus LightGBM. The approval amendment adds an owner-named warm target as a second precondition on increment 2. | Accepted (see decision note) | Modeling ladder |
