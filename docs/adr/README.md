...
| 0013 | [Production Deployment Target: Railway](0013-production-deployment-target.md) | Railway, one `production` environment, two public hostnames and nine private services plus five jobs; the serving bundle and Feast registry are baked into the image; `/readyz` becomes a second unauthenticated path; Postgres traffic stays on the private mesh; the CI k6 gate remains the SLO's only authority and the production canary is explicitly weaker. | Accepted | 3 |
| 0014 | [Request Rate Limiting](0014-request-rate-limiting.md) | Per-`(tenant, subject)` in-process token bucket keyed on the verified token, never on a client IP; 429 with `X-RateLimit-*`; the per-tenant quota column and a Redis-backed shared bucket are the named upgrade path. | Accepted | 3 |
