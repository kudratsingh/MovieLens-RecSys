...
| 0018 | [A Ranker That Reads the Sequence](0018-sequence-aware-ranking.md) | Roadmap Rung 3, proposed in two increments: expose the frozen SASRec encoder's user embedding and its score against each candidate as point-in-time LightGBM features, and only on a named gap add DIN-style target attention in the sidecar. Judged against PR #151's per-route SASRec bundle, not against item-item plus LightGBM. | **Proposed** | Modeling ladder |
