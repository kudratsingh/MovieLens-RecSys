...
**Integrate the durable movie-state, scalable catalog, and frontend-system branches.** The real server-side browser session is complete with refresh/logout/CSRF boundaries and a bypass-disabled Keycloak E2E. Merge Bundles 2–4 in dependency order, then hand off Discover, Browse, Library, detail, Quick Picks, and the finish gate under the contracts in `docs/frontend/implementation-plan.md`.
