...
- ⏳ **Remaining Phase 3 — platform track** (the production deployment work closes none of these except the `prod` half of (e); it deploys what exists rather than extending it) — (a) the ranker training path still builds features with `FeatureIndex` rather than Feast's `get_historical_features` as ADR 0009 specifies — offline/online parity is proven on the served snapshot, not yet on training rows; (b) training-time candidate generation still calls `CandidateIndex.retrieve` with no exclusions at all (noted in PR #64), so the ranker learns over a candidate mix serving no longer produces — either apply the serving filter offline or write down why the difference is acceptable; (c) per-tenant champion model version, quotas, and A/B seed columns on `public.tenants` — the tenant router currently resolves only id, display name, and Redis prefix, and the model sidecar is pinned to one tenant via `MODEL_TENANT_ID`; (d) generic request-audit coverage for authenticated non-prediction endpoints (the audit middleware matches only `/users/{id}/recommendations`; ADR 0012 allows a best-effort/queued policy there only once the durability tradeoff is written down); (e) `docker-compose.{dev,staging}.yml` with `make up-<env>` targets — the `prod` file landed with the deployment work and is the deployed stack as well as its own rehearsal (`make up-prod`, `prod-stores`, `prod-pull`, `prod-release`, `prod-serve`, `prod-seed`, `prod-keycloak-provision`, `prod-verify`, `prod-load`, `prod-backup`, `prod-rollback-rehearsal`, `prod-deploy`, `prod-rollback`, `prod-edge-ca`, `prod-logs`, `prod-reset`, `prod-down`, the stack-touching ones behind a `prod-env-guard` prerequisite that refuses to run without `.env.prod`), so dev and staging are what remain; (f) **the cold-start routing divergence the ADR 0011 cohort found on first use** — the four offline candidate models fall back on "this user has no training history at all" while ADR 0001 and the deployed serving path both fall back below `COLD_START_THRESHOLD = 5`, so buckets h1 and h3 take the learned path offline and `synth_cold_routing_ok` is false on every candidate run. Settling it means running both policies and either moving the models onto the threshold or amending ADR 0001 to say offline retrieval routes on index membership; it is not a side effect of the PR that measured it, because it moves every warm/cold and per-policy metric already logged. Item (d) in particular survives the deployment unchanged: audit coverage is recommendations-only in production too, and `docs/deployment-runbook.md` says so plainly rather than letting non-negotiable #8's wording read as a description of what is running.
...
In parallel on the platform track, the next backend unit is **per-tenant champion columns on `public.tenants`**, which is what unblocks Phase 6's champion/challenger routing: the tenant router resolves only id, display name and Redis prefix today, and the model sidecar is pinned to one tenant through `MODEL_TENANT_ID`, so a second tenant on a second model version is not expressible. After that, in order: the routing divergence the ADR 0011 cohort surfaced (run both policies, then either move the offline models onto `COLD_START_THRESHOLD` or amend ADR 0001 — a decision, not a patch), the Feast-backed ranker training refactor and the training-time candidate-exclusion question PR #64 surfaced alongside it, generic request audits, and the multi-environment compose split. Every new endpoint stays authenticated and uses the RLS-bound request connection.
