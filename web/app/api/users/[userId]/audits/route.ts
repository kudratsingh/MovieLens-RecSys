...
/**
 * Prediction audits back the second step of `Why this?`. They are fetched only
 * when a reader asks for them, which is why they are a browser-reachable route
 * rather than part of the server render.
 */
async function audits(
  ...
  return proxyRecommendationApi(
    accessToken,
    `/users/${userId}/audits${suffix}`,
    {},
    resourceRequestId(request),
  );
}

export const GET = auth(audits);
