...
**One new booster, on the learned route only.** The fallback route keeps the
incumbent booster `05610e60…` on the popularity slate, exactly as 1b composed it,
so the cold slice is bit-identical by construction and the run refuses to write
anything if it is not. Everything else is 1b's: `prepare_shared()`'s prologue,
the identical 154,003 positives, #126's exclusions, seed 42, the same protocol
hash. The only difference between this arm and 1b's warm half is two columns.
...
ranker result. ADR 0018 says so explicitly and this module hard-codes it.
...
from src.feature_contract import LEARNED_ROUTE_FEATURE_COLUMNS, SASREC_SCORE_COLUMNS
...
#: PR #151's per-route bundle. This arm's incumbent, per ADR 0018's "How it is
#: judged": the comparison is a ranker comparison, so the retriever must be held
#: constant on both sides of it.
INCUMBENT_RUN = "566f5309767a4076a4f5e8151be16645"
INCUMBENT_WARM_NDCG = 0.09168799054929602
INCUMBENT_WARM_RECALL = 0.07743102042408873
INCUMBENT_OVERALL_NDCG = 0.21463117342226065
INCUMBENT_OVERALL_RECALL = 0.07748678642635348


def per_route_bundle_result() -> EvalResult:
    """1b as the gate reads it, from the numbers PR #151 published.

    Rebuilt rather than re-run for the same reason ``step1_incumbent_result``
    is: every figure here is recorded, and this module's own cold assertion
...
def run(shared: SharedInputs) -> tuple[EvalResult, dict[str, object]]:
    ...
    # The shape guard PR #151 uses, for the same reason: adding two columns must
    # not change which groups exist. If it did, this arm would be comparing a
    # different training set as well as a different feature set, and the
    # attribution the rung exists to make would be gone.
    #
    # A subsampled smoke run splits at its own cutoff and legitimately has its
    # own shape, so the reproduction claims apply to the full run only — the same
    # rule `prepare_shared` applies to the ADR 0011 cohort. A smoke run proves
    # the pipeline; it never produces a number anybody records.
    full_data = shared.sample_fraction == 1.0
    shape = (len(group_sizes), sum(group_sizes))
    if full_data and shape != STEP1_SASREC_SHAPE:
        raise BundleReproductionError(
            f"rebuilt SASRec training set is {shape} groups/rows, expected "
            f"{STEP1_SASREC_SHAPE} from step 1. Increment 1 adds columns, not groups."
        )
    if not full_data:
        logger.warning(
            "[%s] smoke run at sample fraction %s: %s groups/rows instead of step 1's %s, "
            "and the reproduction guards are skipped. Nothing here is a result.",
            ARM,
    ...
    ranker = LGBMRanker(config=config, feature_columns=list(LEARNED_ROUTE_FEATURE_COLUMNS)).fit(
        features_df, group_sizes, labels
    )
    fit_seconds = time.perf_counter() - started
    logger.info("[%s] booster fit in %.1fs", ARM, fit_seconds)

    # Saved before it is scored, so the weights are a fact independent of
    # whatever the holdout says about them.
    booster_dir = f"{ARM}-{time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())}"
    booster_path, booster_sha256 = _save_booster_create_only(
        ranker, shared.booster_root / booster_dir
    )
    logger.info("[%s] booster %s sha256=%s", ARM, booster_path, booster_sha256)
    ...
    recommendations = rank_by_route(
        ...
        warm_ranking_features=holdout_feature_builder(shared, features),
    )
    rank_seconds = time.perf_counter() - started
    result = _evaluate(shared, recommendations)
    _log_slices(ARM, result)

    # The fallback route is untouched by construction, so its slice must come
    # back as the same float 1b produced — not to six places, to the last bit.
    # A near miss would mean the routing or the feature path moved, and the warm
    # number would then not be attributable to the two new columns.
    for label, actual, expected in (
        ...
        if full_data and actual != expected:
            raise BundleReproductionError(
                f"{ARM} {label} is {actual!r}, expected the per-route bundle's {expected!r} "
                "exactly. Increment 1 changes the learned route only; a different cold "
                "slice means something else moved with it."
            )

    importances = ranker.feature_importances(importance_type="gain")
    logger.info("[%s] gain importances: %s", ARM, importances)
...
def main() -> None:
    ...
    logger.info("ADR 0001 gate, %s vs the PR #151 per-route bundle:", ARM)
